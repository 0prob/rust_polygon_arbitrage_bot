...
fn ln(a: U256) -> U256 {
    ...
    let o20 = one_20();
    for &(threshold, factor) in LN_STEPS.iter() {
        if av >= threshold {
            av = (av * o20) / factor;
            sum += threshold;
        }
...
pub fn log_exp_exp(x: U256) -> U256 {
    ...
    for &(threshold, factor) in EXP_STEPS.iter() {
        if xv >= threshold {
            xv -= threshold;
...
const LN_A0: U256 = U256::from_limbs([171843153341448192, 17670479068478958691, 114249481722274167, 0]);
const LN_A1: U256 = U256::from_limbs([17696838799657497472, 338008108, 0, 0]);

const LN_STEPS: [(U256, U256); 10] = [
    (U256::from_limbs([8713275248247570432, 173, 0, 0]), U256::from_limbs([17871857890508685312, 428059064879743, 0, 0])),
    (U256::from_limbs([13580009660978561024, 86, 0, 0]), U256::from_limbs([12108528782385981184, 48171701, 0, 0])),
    (U256::from_limbs([6790004830489280512, 43, 0, 0]), U256::from_limbs([14861217100182911056, 16159, 0, 0])),
    (U256::from_limbs([12618374452099416064, 21, 0, 0]), U256::from_limbs([18025501570106181090, 295, 0, 0])),
    (U256::from_limbs([15532559262904483840, 10, 0, 0]), U256::from_limbs([1035846944682958083, 40, 0, 0])),
    (U256::from_limbs([7766279631452241920, 5, 0, 0]), U256::from_limbs([13573765813970800912, 14, 0, 0])),
    (U256::from_limbs([13106511852580896768, 2, 0, 0]), U256::from_limbs([17298174480336401757, 8, 0, 0])),
    (U256::from_limbs([6553255926290448384, 1, 0, 0]), U256::from_limbs([17722077226516838711, 6, 0, 0])),
    (U256::from_limbs([12500000000000000000, 0, 0, 0]), U256::from_limbs([2634380864425321987, 6, 0, 0])),
    (U256::from_limbs([6250000000000000000, 0, 0, 0]), U256::from_limbs([14215725523238184876, 5, 0, 0])),
];

const EXP_X0: U256 = U256::from_limbs([0, 0, 0, 128]);
const EXP_X1: U256 = U256::from_limbs([0, 0, 0, 64]);

const EXP_A0: U256 = U256::from_limbs([171843153341448192, 17670479068478958691, 114249481722274167, 0]);
